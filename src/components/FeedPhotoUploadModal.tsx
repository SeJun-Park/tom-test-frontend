...
import { createFeedPhoto, getUploadURL, uploadImage } from "../api";
...
interface IFeedPhotoUploadForm {
    file : FileList;
}
...
export default function FeedPhotoUploadModal ( props : FeedPhotoUploadModalProps ) {

    const { teamPk } = useParams();
    const feedPk = String(props.pk)

    const { watch, register, handleSubmit, formState : {errors}, reset : feedPhotoUploadFormReset } = useForm<IFeedPhotoUploadForm>();

    const toast = useToast();
    const queryClient = useQueryClient()

    const feedPhotoUploadMutation = useMutation(createFeedPhoto, {
        onSuccess : (data) => {
            toast({
                status : "success",
                title : "사진 등록 완료",
            ...
            feedPhotoUploadFormReset();
            queryClient.refetchQueries(["teamFeeds"])
        },
        onError : (error) => {
            console.log("feedPhotoUploadMutation is onError")
            console.log(error)
        }
    })
    ...
    const uploadImageMutation = useMutation(uploadImage, {
        onSuccess : ({result}) => {
            ...
            if(teamPk) {
                feedPhotoUploadMutation.mutate({
                    teamPk,
                    feedPk,
    ...
    {/* motionPreset prop을 이용하면 모달이 나타나는 모양을 지정할 수 있음, default는 "scale" */}
        ...
        <ModalContent> 
            <ModalHeader> 피드 사진 등록하기 </ModalHeader>
            <ModalCloseButton />
            <ModalBody as="form" onSubmit={handleSubmit(onSubmit)}>
                <VStack>
                    ...
                    {feedPhotoUploadMutation.isError ? (<Text color={"red.100"} textAlign={"center"} fontSize={"sm"}> 문제가 발생했습니다. </Text>) : null}
                    <Button type="submit" isLoading={feedPhotoUploadMutation.isLoading || uploadImageMutation.isLoading || getUploadURLMutation.isLoading} size={"md"} width="100%" backgroundColor={"main.500"} color={"white"}> 등록하기 </Button>
