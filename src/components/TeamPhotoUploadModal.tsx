...
interface ITeamPhotoUploadForm {
    file : FileList;
}
...
export default function TeamPhotoUploadModal ( props : TeamPhotoUploadModalProps ) {
    ...
    const teamPhotoUploadMutation = useMutation(teamPhotoUpload, {
        onSuccess : (data) => {
            toast({
                status : "success",
    ...
    {/* motionPreset prop을 이용하면 모달이 나타나는 모양을 지정할 수 있음, default는 "scale" */}
        ...
        <ModalContent> 
            <ModalHeader> 팀 프로필 사진 등록하기 </ModalHeader>
            <ModalCloseButton />
            <ModalBody as="form" onSubmit={handleSubmit(onSubmit)}>
                <VStack>
                    ...
                    {teamPhotoUploadMutation.isError ? (<Text color={"red.100"} textAlign={"center"} fontSize={"sm"}> 문제가 발생했습니다. </Text>) : null}
                    <Button type="submit" isLoading={teamPhotoUploadMutation.isLoading || uploadImageMutation.isLoading || getUploadURLMutation.isLoading} size={"md"} width="100%" backgroundColor={"main.500"} color={"white"}> 등록하기 </Button>
