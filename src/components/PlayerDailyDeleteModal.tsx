...
interface PlayerDailyDeleteModalProps {
    isOpen : boolean;
    onClose : () => void;
    playerPk : number;
}

export default function PlayerDailyDeleteModal ( props : PlayerDailyDeleteModalProps ) {

    const playerPk = String(props.playerPk)
    ...
    const playerDeleteMutation = useMutation(playerDelete, {
        onSuccess : (data) => {
            console.log("playerDaily delete successful")
            // data.ok
            toast({
                title : "용병 삭제 성공",
                status : "success"
    ...
    {/* motionPreset prop을 이용하면 모달이 나타나는 모양을 지정할 수 있음, default는 "scale" */}
        ...
        <ModalContent> 
            <ModalHeader> 용병을 삭제하시겠습니까? </ModalHeader>
            <ModalCloseButton />
            <ModalBody>
                <VStack>
