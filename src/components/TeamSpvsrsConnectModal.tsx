...
import { teamSpvsrsConnect, teamSpvsrsConnecting } from "../api";
...
export default function TeamSpvsrsConnectModal ( props : TeamSpvsrsConnectModalProps ) {
    ...
    const onSubmit = () => {
        if(teamPk) {
            teamSpvsrsConnectMutation.mutate({ teamPk })
        }
    }
    ...
    {/* motionPreset prop을 이용하면 모달이 나타나는 모양을 지정할 수 있음, default는 "scale" */}
        ...
        <ModalContent> 
            <ModalHeader> 이름 : {props.userName} <br/> 팀 관리자로 등록을 요청을 수락하시겠습니까? </ModalHeader>
            <ModalCloseButton />
